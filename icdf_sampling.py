...
def createicdf(pdf=None, mv=None, mfv=None, Mthresh=10**10.5, M0=1e13):
    ...
    ncum = np.cumsum(nbin)
    ncum /= ncum.max()
    icdf = interpolate(ncum, 0.5*(lmm[:-1] + lmm[1:]))    
    return icdf

def sampleicdf(icdf, N0, seed=11):
    np.random.seed(seed)
    ran = np.random.uniform(0, 1, N0)
    hmatch = 10**(icdf(ran))
    hmatch.sort()
...
def assigngalaxymass(galcat, hmass, mbins=None, verbose=False, seed=100, Mthresh=10**10.5, M0=1e13):
    '''
    ...
    if mbins is None:
        mbins  = np.logspace(np.log10(hmass.min()), np.log10(hmass.max()), 10)[::-1]
    ...
    for i in range(len(mbins)-1):
        ...
        icdfc = createicdf(pdf = lambda x: csmfcen(x, mh=mh), Mthresh=Mthresh, M0=mh)
        galmass[cens] = sampleicdf(icdfc, cens.sum(), seed=seed*i)
    ...
    #     icdfs = sampler(pdf = lambda x: icdf.csmfsf(x, mh=mh), M0=mh)
        galmass[sats] = sampleicdf(icdfs, sats.sum(), seed=seed*i)
